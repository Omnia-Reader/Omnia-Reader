...
Given a converged GitHub destination, when a reader creates a highlight, its local save remains immediate and its remote attempt starts within 150 ms after activity becomes quiet. The attempt reads and writes only the affected annotation document and does not synchronize schema, publications, logical books, bookmarks, or progress.
...
- **FR-001**: Highlight and bookmark synchronization MUST be scheduled 150 ms after the last interactive mutation unless provider backoff requires a longer delay.
...
- **FR-004**: A targeted attempt MUST read only each affected remote document and MUST NOT list unrelated prefixes or run schema, publication, or logical-book workers.
...
- **SC-001**: An eligible highlight waits at most 150 ms of local quiet time before its network attempt begins.
- **SC-002**: One new eligible highlight causes one destination revision probe, one targeted document read, and one document write at the browser-to-gateway boundary, with no prefix listing or complete verification pass.
