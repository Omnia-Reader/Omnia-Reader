...
Use one provider-neutral scheduler for both coalesced local commits and remote GitHub revision discovery. Page-progress activity uses a two-second trailing debounce, while interactive annotations and bookmarks use their separate latency budget. A visible, online GitHub client runs the existing change-aware worker every ten seconds; its trusted checkpoint reduces unchanged attempts to one `/revision` request.
...
- Default progress quiet interval: 2,000 ms.
...
| ---------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| Scheduler  | two-second progress debounce, interactive mutation budget, ten-second revision check, lifecycle/backoff | `npx nx test sync-core --skip-nx-cache`    |
