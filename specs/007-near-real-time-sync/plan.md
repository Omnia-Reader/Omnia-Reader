...
Use one provider-neutral scheduler for both rapid local commits and remote GitHub revision discovery. Local reading activity uses a one-second trailing debounce. A visible, online GitHub client runs the existing change-aware worker every ten seconds; its trusted checkpoint reduces unchanged attempts to one `/revision` request.
...
- Default quiet interval: 1,000 ms.
...
| ---------- | ----------------------------------------------------------------- | ------------------------------------------ |
| Scheduler  | one-second debounce, ten-second revision check, lifecycle/backoff | `npx nx test sync-core --skip-nx-cache`    |
